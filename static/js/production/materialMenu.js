...
(function( window ) {
  ...
  Menu.prototype._initItemTransitions = function() {
    ...
    for ( var i = 0; i < len; i++ ) {
	  ...
	  var menuLink = this.menuItemLinks[i];
	  menuLink.setAttribute('num',num);
  ...
  Menu.prototype._touchEffectHandler = function( menuItemLink ) {
    ...
    menuItemLink.addEventListener( "click", function(e) {
      ...
      touchEffectElement.classList.add("animating");
		...
		var lastIdx = $('.tab-show').length - 1;
		var curNum = parseInt(e.currentTarget.getAttribute('num'));
		if( $('.tab-show')[lastIdx].id != 'tab'+curNum ){
			//menu btn change
			$('.mm-menu__click')[0].classList.remove('mm-menu__click');
			e.currentTarget.classList.add('mm-menu__click');
